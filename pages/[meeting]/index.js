...
export default function Home(props) {
    ...
    return (
        <>
            ...
            <PosterDialog open={open} handleClose={() => setOpen(false)} />
        </>
    )
}

const PosterDialog = ({ open, handleClose }) => {
    return (
        <Dialog onClose={handleClose} open={open} fullScreen={true} PaperProps={{
            style: {
                backgroundColor: 'transparent',
                boxShadow: 'none',
            },
        }}>
            <DialogTitle style={{ display: "flex", justifyContent: "flex-end", zIndex: 2 }}>
                <IconButton
                    aria-label="close"
                    onClick={handleClose}
